...
static void play(){
	...
	while(1){
		if(!key_queue_is_empty()) {
			...
			else if(next.scancode == 75 && ploc%VGA_W!=0) 		ploc--;	//left
			else if(next.scancode == 77 && ploc%VGA_W!=VGA_W-8)	ploc++;	//right
		}
		i++;
		if(i==1000000){
			i=0;
			if(pxloc == 0) dir+=2;
			if(pxloc == VGA_W-1) dir-=2;
			switch(dir){
				case 0:
					pyloc-=1;
					pxloc-=1;
			...
			if(pyloc==VGA_H) {pyloc=VGA_H/2; pxloc=VGA_W/2; dir= 4;}
			else if(pyloc==VGA_H-1){
				if(pxloc>=ploc+3 && pxloc<=ploc+4) dir = (dir+3)%6;
				else if(pxloc>=ploc && pxloc<=ploc+2) dir = 0;
				else if(pxloc>=ploc+5 && pxloc<=ploc+7) dir = 2;
			...
			oloc=(oloc<0)?0:oloc;
			oloc=(oloc>VGA_W-8)?VGA_W-8:oloc;
			drawboard(ploc, oloc, pxloc, pyloc);
		}
